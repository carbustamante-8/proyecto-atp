...
// 1. AGREGAMOS EL CAMPO AL TIPO DE DATO
type OrdenDeTrabajo = {
  id: string;
  patente: string;
  ...
  fechaCierreAdministrativo?: { _seconds: number }; // <--- ¡NUEVO CAMPO!
...
export default function GeneradorReportesPage() {
  ...
  const handleExportExcel = () => {
    ...
    const datosParaExcel = ordenesFiltradas.map(ot => ({
      ...
      "Fecha Cierre Admin": formatFecha(ot.fechaCierreAdministrativo), // <--- ¡AQUÍ ESTÁ!
  ...
  return (
    <div className="p-8 text-gray-900">
      ...
      <form onSubmit={handleGenerarReporte} className="bg-white p-6 rounded-lg shadow-md mb-8 grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
        <div>
          <label htmlFor="filtroFechaInicio" className="block text-sm font-medium text-gray-700">Fecha Inicio</label>
        ...
        <div>
          <label htmlFor="filtroFechaFin" className="block text-sm font-medium text-gray-700">Fecha Fin</label>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              {/* 3. AGREGAMOS LA COLUMNA VISUAL EN LA TABLA */}
            ...
            {loading ? (
              ...
              ordenesFiltradas.map(ot => (
                <tr key={ot.id} className={`${ot.estado === 'Anulado' ? 'bg-red-50 opacity-60' : ''}`}>
                  ...
                  {/* 4. MOSTRAMOS EL DATO AQUÍ */}
                  <td className="px-6 py-4 text-sm text-blue-700 font-medium">
                    {formatFecha(ot.fechaCierreAdministrativo)}
                  </td>
              ...
              <tr><td colSpan={7} className="px-6 py-4 text-center">No se encontraron OTs.</td></tr>
            )}
          </tbody>
        </table>
      </div>
