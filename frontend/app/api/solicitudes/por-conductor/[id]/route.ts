...
/**
 * Función GET: Obtiene todas las solicitudes de un conductor
 * y las cruza con el estado de la OT relacionada.
 */
export async function GET(request: NextRequest, context: Context) {
  ...
  try {
    ...
    // 1. Busca todas las solicitudes hechas por este conductor
    const solicitudesSnapshot = await adminDb.collection('solicitudes')
      .where('id_conductor', '==', idConductor)
      .orderBy('fechaCreacion', 'desc') // Mostrar las más nuevas primero
      .get();
    ...
    // 2. Prepara un array para la respuesta
    const solicitudesConEstado = [];

    // 3. Itera sobre cada solicitud y busca el estado de la OT (si existe)
    for (const doc of solicitudesSnapshot.docs) {
      const solicitud = doc.data();
      let estadoOT = null; // Estado por defecto
      let fechaIngresoTaller = null; // Fecha de ingreso real
