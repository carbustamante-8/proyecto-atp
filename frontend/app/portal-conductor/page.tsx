...
// (CÓDIGO ACTUALIZADO: Sin subida de fotos, solo texto)
...
export default function PortalConductorPage() {
  ...
  const fetchMisSolicitudes = async (conductorId: string) => {
    ...
    try {
      ...
      // Filtramos solicitudes antiguas/canceladas si es necesario
      const solicitudesActivas = data.filter(sol => {
        if (sol.estadoOT === 'Cerrado' || sol.estadoOT === 'Anulado') return false; 
        return true; 
      });
  ...
  // --- handleSolicitud (SIMPLIFICADO: Solo texto) ---
  const handleSolicitud = async (e: React.FormEvent) => {
    ...
    const promise = (async () => {
      // Crear la solicitud (Ya no enviamos fotoURL)
      const response = await fetch('/api/solicitudes', {
        ...
        body: JSON.stringify({
          ...
          // fotoEvidenciaUrl: null, // Eliminado
        }),
      });
  ...
  const getEstadoConductor = (sol: SolicitudConEstado): { texto: string, color: string } => {
    if (sol.estadoOT === 'Finalizado') return { texto: '¡LISTO PARA RETIRO!', color: 'text-green-600 font-bold' };
    if (sol.estadoOT === 'En Progreso') return { texto: 'En Taller (En Progreso)', color: 'text-yellow-600' };
    if (sol.estadoOT === 'Pendiente') return { texto: 'En Taller (Pendiente de Mecánico)', color: 'text-yellow-600' };
    if (sol.estadoOT === 'Agendado') return { texto: 'Agendado (Esperando hora)', color: 'text-blue-600' };
    if (sol.estadoSolicitud === 'Pendiente') return { texto: 'Pendiente de Aprobación (Admin)', color: 'text-gray-500' };
    return { texto: 'Procesado', color: 'text-gray-500' };
  };

  if (authLoading || !userProfile) return <div className="p-8 text-gray-900">Validando sesión...</div>;
  ...
  return (
    <div className="p-8 text-gray-900 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8"> 
      ...
      <div className="md:col-span-1 space-y-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          {loadingVehiculo ? (
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              ...
              <div><span className="text-sm text-gray-500">Marca</span><p className="font-medium">{miVehiculo.marca || 'N/A'}</p></div>
              <div><span className="text-sm text-gray-500">Modelo</span><p className="font-medium">{miVehiculo.modelo || 'N/A'}</p></div>
              <div><span className="text-sm text-gray-500">Año</span><p className="font-medium">{miVehiculo.año || 'N/A'}</p></div>
              <div><span className="text-sm text-gray-500">Color</span><p className="font-medium">{miVehiculo.color || 'N/A'}</p></div>
            </div>
          ) : (
        ...
        {/* Sección 2: Solicitar Mantenimiento (LIMPIA) */}
        {miVehiculo && (
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <form onSubmit={handleSolicitud} className="space-y-4">
              ...
              <button
                ...
                className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-lg font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 transition-colors"
              >
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loadingSolicitudes ? (
                  ...
                  misSolicitudes.map(sol => {
                    ...
                    return (
                      <tr key={sol.id}>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-700">{sol.descripcion}</td>
                  ...
                  <tr><td colSpan={3} className="px-6 py-4 text-center text-gray-500">No tienes solicitudes activas.</td></tr>
                )}
              </tbody>
            </table>
          </div>
